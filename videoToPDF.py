...
class MacOSScreenCapture:
    ...
    def capture_frame(self):
        ...
        try:
            # Use PIL's ImageGrab which handles macOS screen capture correctly
            if self.region:
                x, y, w, h = self.region
...
class SlideCapture:
    ...
    def auto_detect_window(self, keywords):
        ...
        try:
            ...
            for window in window_list:
                ...
                # Skip system windows and windows without titles
                if not title or owner in ignored_apps:
                    continue
            ...
            if matches:
                ...
                print(f"Found window: {best_match['title']} ({best_match['owner']})")
            ...
            for window in window_list:
                ...
                owner = window.get('kCGWindowOwnerName', '')
                bounds = window.get('kCGWindowBounds', {})
                if title and owner not in ignored_apps and bounds.get('Width', 0) >= 200:
                    print(f"  - {title} ({owner})")
    ...
    def start_capture(self, region=None):
        ...
        if region is None:
            region = self.get_full_screen_region()
            print("Capturing full screen")
        
        # Initialize screen capture
        capture = MacOSScreenCapture(region)
...
def main():
    ...
    keywords = ["Box", "YouTube", "Video", "Chrome", "Safari", "Firefox", "Zoom", "Google Meet", "Microsoft Teams", "Meeting"]
    ...
    print("\n" + "=" * 50)
    print("IMPORTANT: Grant screen recording permissions if prompted")
